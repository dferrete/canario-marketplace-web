    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await register({
                ...
                telefone: phone,
                email,
                senha: password // Envia 'senha' para casar com a API
            });
            setIsSuccess(true);
            setTimeout(() => {
                router.push("/");
                router.refresh(); // Refresh state so AuthContext kicks in immediately
            }, 2000);
        } catch (err: any) {
            setError(err?.response?.data?.message || err.message || "Falha ao realizar cadastro. Tente novamente.");
        }
    };
    ...
    if (isSuccess) {
        return (
            <div className="flex min-h-[80vh] items-center justify-center p-4">
                <Card className="w-full max-w-md border-border/50 bg-green-50/50 backdrop-blur-xl shadow-2xl animate-in fade-in zoom-in duration-500">
                    <CardContent className="flex flex-col items-center justify-center p-8 space-y-4 text-center">
                        <CheckCircle2 className="h-16 w-16 text-green-500" />
                        <h2 className="text-2xl font-bold tracking-tight text-green-700">Conta Criada!</h2>
                        <p className="text-green-600/80">Seu cadastro foi realizado com sucesso. Autenticando e redirecionando para a vitrine...</p>
                    </CardContent>
                </Card>
            </div>
        );
                    ...
                    <div className="px-6 pb-6">
                        <div className="relative mb-6">
                            ...
                            <div className="relative flex justify-center text-xs uppercase">
                                <span className="bg-surface px-2 text-muted-foreground">{t("auth.register.orRegisterWith")}</span>
                            </div>
                        </div>
